...
const TvDetails = () => {
  ...
  return info ? (
    ...
    >
      {/* Top Nav Icons */}
      <nav className="w-full text-zinc-400 text-2xl md:text-3xl flex gap-6 md:gap-12 p-5 md:p-8">
      ...
      <div className="flex flex-col lg:flex-row gap-8 px-5 md:px-[5%] items-start">
        {/* Poster */}
        <img
          className="w-full lg:w-[40vh] h-auto object-cover rounded shadow-lg"
          src={
        ...
        <div className="text-white flex-1">
          <h1 className="text-3xl md:text-4xl lg:text-5xl font-bold">
            {info.detail.name}
            <small className="text-lg md:text-2xl ml-2 text-zinc-300">
              ({info.detail.first_air_date?.split("-")[0]})
            </small>
          </h1>

          <div className="flex flex-wrap gap-4 py-3 text-sm md:text-base text-zinc-300">
            <h1>
          ...
          <div className="text-sm md:text-base mt-2 text-zinc-300">
            <div className="flex flex-wrap gap-4 font-medium mb-3">
              <span>Status: {info.detail.status}</span>
              <span>Seasons: {info.detail.number_of_seasons}</span>
              <span>Episodes: {info.detail.number_of_episodes}</span>
            </div>

            <h2 className="text-xl md:text-2xl mb-1">{info.detail.tagline}</h2>
          ...
          <div className="mt-5">
            <Link to={`${pathname}/trailer`}>
              <button className="px-4 py-2 bg-[#6556cd] rounded text-zinc-100 text-sm md:text-base">
                Watch Trailer
              </button>
            </Link>
          </div>
      ...
      <div className="px-5 md:px-10 mt-10">
        <h2 className="text-2xl md:text-4xl font-semibold text-zinc-400 mb-4">
          Recommendations
        </h2>
        <HorizontalCards
          data={
